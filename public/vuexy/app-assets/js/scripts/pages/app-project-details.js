$(function() {
        ...
        var dtMember = dtMemberTable.DataTable({
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        ...
                        return `<div data-toggle="tooltip" data-popup="tooltip-custom" data-placement="top" title="" class="avatar pull-up my-0" data-original-title="${row.name}">
                                ...
                                <span class="font-weight-bold">${row.name}</span> ${leader}
                                `;
                    }
                },
                {
            ...
            buttons: [{
                ...
                },
                {
                    ...
                    action: function(e, dt, button, config) {
                        $(choose_leader_modal).modal('show');
                    }
                }
            ],
    ...
    dtMember.on('click', '.btn-delete', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const deleteMember = await $.post(`/projects/remove-member`, {
                    _token: $('meta[name=csrf-token]').attr('content'),
                    id: dtMemberTable.data('id'),
                    member_id
                });
                if (deleteMember.success) {
                    toastr['success'](deleteMember.msg, 'Deleted!', {
                        closeButton: true,
                        tapToDismiss: false,
                        rtl: isRtl
                    });
                    dtMember.ajax.reload();
                }
            }
        });
    });
    ...
    add_member_modal.on('submit', 'form', function(e) {
        e.preventDefault();
    ...
    choose_leader_modal.on('submit', 'form', function(e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: async function(resp) {
                if (resp.success) {
                    ...
                    choose_leader_modal.modal('hide');
                    dtMember.ajax.reload();
                }
            }
        })
