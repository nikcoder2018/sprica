    ...
    function assignTask(option) {
        ...
        if ($(option.element).data('img') == '') {
            ...
            <div class="avatar bg-light-warning mr-50">
                <div class="avatar-content">${$(option.element).data('prefix')}</div>
            </div>
          `;
        } else {
            profileImg = `
            <img class="d-block rounded-circle mr-50" src="${$(option.element).data('img')}" height="26" width="26" alt="${option.text}">
            `;
        }
        var $person =
        ...
        newTaskForm.on('submit', function(e) {
            ...
            if (isValid) {
                ...
                $.ajax({
                    url: '/todo',
                    type: 'POST',
                    data: $(this).serialize() + '&description=' + todoDescEditor.root.innerHTML,
                    success: function(resp) {
                        if (resp.success) {
                            var selectedDate = new Date(resp.date_due),
                                month = new Intl.DateTimeFormat('en', { month: 'short' }).format(selectedDate),
                                day = new Intl.DateTimeFormat('en', { day: '2-digit' }).format(selectedDate),
                                todoDate = month + ' ' + day;
